...
import NotificationCenter, { useUnreadCount } from './NotificationCenter'
...
export default function TopBar({ onMenuClick, title }: TopBarProps) {
  ...
  const unread = internalUnread ?? liveUnread

  function openNotif() {
    const r = bellRef.current?.getBoundingClientRect()
    if (r) setNotifAnchor({ top: r.bottom + 8, right: Math.max(12, window.innerWidth - r.right) })
    setNotifOpen(true)
  }
      ...
      <header className="sticky top-0 z-30 flex h-14 sm:h-16 items-center justify-between border-b border-border-default bg-bg-topbar px-3 sm:pl-4 sm:pr-6 lg:pl-6">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <button
            ...
            title="Search (Ctrl+K)"
            className="h-9 w-9 sm:h-10 sm:w-10 flex items-center justify-center rounded-full text-text-muted hover:bg-bg-card-hover hover:text-text-primary transition-colors"
          >
            <Search size={18} />
          </button>
          {isSuperAdmin && (
            <button
              ...
              onClick={openNotif}
              title="Notifications"
              className="relative h-9 w-9 sm:h-10 sm:w-10 flex items-center justify-center rounded-full text-text-muted hover:bg-bg-card-hover hover:text-text-primary transition-colors"
            >
              <Bell size={18} />
              {unread > 0 && (
                <span className="absolute top-1 right-1 min-w-[16px] h-[16px] px-1 rounded-full bg-red-600 text-white text-[10px] font-bold flex items-center justify-center ring-2 ring-white">
                  {unread > 99 ? '99+' : unread}
                </span>
              )}
            </button>
