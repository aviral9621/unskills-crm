...
export default function ExpenseCategoriesPage() {
  ...
  async function save() {
    ...
    try {
      if (editId) {
        const { error } = await supabase.from('uce_expense_categories').update({ name: name.trim() }).eq('id', editId)
        if (error) throw error
        toast.success('Category updated')
      } else {
        const { error } = await supabase.from('uce_expense_categories').insert({ name: name.trim(), is_active: true, is_system: false })
        if (error) {
  ...
  async function toggleActive(c: Cat) {
    try {
      const { error } = await supabase.from('uce_expense_categories').update({ is_active: !c.is_active }).eq('id', c.id)
      if (error) throw error
  ...
  async function doDelete() {
    ...
    setDeleting(true)
    try {
      const { error } = await supabase.from('uce_expense_categories').delete().eq('id', delTarget.id)
      if (error) {
        // Likely FK constraint — fall back to soft-disable
        const { error: e2 } = await supabase.from('uce_expense_categories').update({ is_active: false }).eq('id', delTarget.id)
        if (e2) throw e2
        toast.warning('Category was used by existing expenses — disabled instead of deleted')
      } else {
        toast.success('Category deleted')
      }
      setCats(p => p.filter(x => x.id !== delTarget.id || (x.id === delTarget.id && !x.is_active)))
      load()
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/admin/reports/expenses')} className="p-2 rounded-lg hover:bg-gray-100">
          <ArrowLeft size={18} className="text-gray-600" />
        </button>
        <div className="flex-1">
          <h1 className="text-lg sm:text-2xl font-bold text-gray-900 font-heading">Expense Categories</h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Manage which categories appear when adding expenses</p>
        </div>
          ...
          <ul className="divide-y divide-gray-100">
            {cats.map(c => (
              <li key={c.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50/60">
                <Tag size={14} className={c.is_active ? 'text-red-500' : 'text-gray-300'} />
      ...
      <ConfirmDialog open={!!delTarget} onClose={() => setDelTarget(null)} onConfirm={doDelete}
        ...
        message={`Delete "${delTarget?.name}"? If it's used by existing expenses it will be hidden instead.`}
